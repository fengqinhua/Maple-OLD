...
//using Maple.Web.Environment.Extensions.Helpers;
using Maple.Web.Environment.Extensions.Models;
using Maple.Localization;
using Maple.Logging;

namespace Maple.Web.Environment.Features {
                    ...
                    (currentFeatureId, featuresState) => {
                        ...
                        return feature.Key.Dependencies
                                      .Select(fId => {
                                          ...
                                          if (states.Count == 0) {
                                              throw new MapleWebCoreException(T("Failed to get state for feature {0}", fId));
                                          }

                                          if (states.Count > 1) {
                                              throw new MapleWebCoreException(T("Found {0} states for feature {1}", states.Count, fId));
                                          }
