...
//using Maple.Web.ContentManagement.Records;
    ...
    public class CompositionStrategy : ICompositionStrategy {
            ...
            foreach (var shellFeature in features) {
                ...
                if (!availableFeatures.ContainsKey(shellFeature)) {
                    ...
                    if(dependentFeatureDescriptor != null)
                        throw new MapleWebCoreException(
        ...
        private static IEnumerable<string> GetExcludedTypes(IEnumerable<Feature> features) {
            ...
            foreach (var feature in features) {
                foreach (var type in feature.ExportedTypes) {
                    foreach (MapleSuppressDependencyAttribute replacedType in type.GetCustomAttributes(typeof(MapleSuppressDependencyAttribute), false)) {
        ...
        private static IEnumerable<Feature> BuiltinFeatures() {
            yield return new Feature {
                ...
                ExportedTypes =
                    typeof(MapleStarter).Assembly.GetExportedTypes()
                    .Where(t => t.IsClass && !t.IsAbstract)
                    .Except(new[] { typeof(DefaultMapleHost) })
                    .ToArray()
            };
        }
