        ...
        protected override IEnumerable<ModelValidator> GetValidators(ModelMetadata metadata, ControllerContext context, IEnumerable<Attribute> attributes) {
            ...
            foreach (var attribute in attributes) {
                ...
                if ( _validationAttributes.TryGetValue(attribute.GetType(), out localizedAttribute) ) {
                    localizedAttributes.Add(localizedAttribute((ValidationAttribute)attribute, NullLocalizer.Instance /*tProvider.Value*/));
                }
                else {
                    ...
                    if ( validationAttribute != null ) {
                        ...
                        if ( propertyInfo != null ) {
                            propertyInfo.SetValue(attribute, NullLocalizer.Instance/*Provider.Value*/, null);
                        }
                    }
                    
                    if ( attribute is DisplayNameAttribute ) {
                        metadata.DisplayName = NullLocalizer.Instance(metadata.DisplayName).Text;// tContainer.Value(metadata.DisplayName).Text;
                    }
